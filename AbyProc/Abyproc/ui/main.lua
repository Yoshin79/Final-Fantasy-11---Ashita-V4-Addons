...
function ui.render_controls()
    ...
    if imgui.CollapsingHeader('Controls', ImGuiTreeNodeFlags_DefaultOpen) then
        ...
        if imgui.Button("Show in Chat") then
            -- Just for display
            local procs_module = require('core.procs');
            local vulnerability = procs_module.get_vulnerability();
...
function ui.render()
    ...
    if target.staggered and os.time() - target.stagger_time > 30 then
        ...
        -- Also reset vulnerability if it was set
        local procs_module = require('core.procs');
        if procs_module.current_vulnerability then
            procs_module.current_vulnerability = nil;
